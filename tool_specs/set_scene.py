...
SPEC = ToolSpec(
    ...
    description=(
        ...
        "SAVE SCENES FOR:\n"
        "✓ Multi-step workflows (TV on → app → media)\n"
        "✓ Ambiguous entity discovery (which Plex client, which TV)\n"
        ...
        "✗ Simple control (turning on lights - no scene needed)\n"
        "✗ Unchanged replay (auto-skips duplicate)\n\n"
        "VIBE WORKFLOW:\n"
        "1. User says 'make it cozy'\n"
        "2. ask_user('Which lights dim? Music? Temperature?')\n"
        "3. Execute their preferences\n"
        "4. ask_user('Save as cozy scene?') → if yes, set_scene\n\n"
        "STEPS FORMAT:\n"
        "✓ Service: {\"type\":\"service_call\",\"service\":\"...\",\"data\":{...}}\n"
        "✓ Delay: {\"type\":\"delay\",\"seconds\":N}\n"
        "✓ Tool: {\"type\":\"tool_call\",\"tool\":\"play_plex_media\",\"args\":{...}} (sequence-safe only)\n"
        "✓ Guards: only_if_state for conditional steps (state/attribute checks)\n\n"
        "VALUE: Scenes enable 2-call execution (get_scene+search in Call-1, run_sequence in Call-2).\n"
