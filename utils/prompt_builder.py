...
async def build_system_prompt(
    ...
    system_prompt = (
        ...
        "FAST PATH (2-call pattern for multi-step requests):\n"
        "- Call-1 GATHERING: Fire parallel lookups (scene exact+similar, devices, media) to collect all options\n"
        "  Example: get_scene(intent='play_plex_gym',k=1) + get_scene(intent='play_plex',k=3) + search_devices(...) + search_plex(...)\n"
        "- Call-2 EXECUTION: Pick best option, execute once (run_sequence or direct control), pre-generate voice response\n"
        "- Multi-step workflows (media playback, app switching): ALWAYS search scenes in Call-1\n"
        "- Simple requests (turn on lights): Skip scenes, execute directly\n"
