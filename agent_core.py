...
async def plan_execute(
    ...
    system_prompt = (
        ...
        "SCENE MEMORY (if enabled):\n"
        "- For multi-device or scene-like requests ('movie time', 'play X in room', 'cozy', 'good night'), FIRST call get_scene\n"
        "- If get_scene returns commands_list with confidence >0.6, use it with run_sequence\n"
        "- If no scene found, compose steps manually using search_devices and control_device\n"
        "- After successful execution, call set_scene to save the routine for next time\n"
        "- Example flow: get_scene → (if found) run_sequence → set_scene to reinforce\n"
